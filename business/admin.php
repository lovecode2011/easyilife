...
if( 'edit' == $opera ) {
    ...
    $get_admin = 'select * from `'.DB_PREFIX.'admin` where business_account = \''.$_SESSION['business_account'].'\' and account = \''.$account.'\' limit 1';
    ...
    if( '' == $email ) {
        ...
        $check_email = 'select * from '.$db->table('business').' where `email`=\''.$email.'\'';
        if( $db->fetchRow($check_email) ) {
            show_system_message('电子邮箱已被占用，请使用其他邮箱', array());
            exit;
        }
...
if( 'add' == $opera ) {
    ...
    if( '' == $account ) {
        ...
        exit;
    } else if( !preg_match('#([a-z][A-Z][0-9])*@'.$_SESSION['business_account'].'#', $account) ) {
        show_system_message('帐号格式不正确', array());
        exit;
    } else {
        $account = $db->escape(htmlspecialchars($account));
        $check_account = 'select `account` from `'.DB_PREFIX.'admin` where business_account = \''.$_SESSION['business_account'].'\' and `account`=\''.$account.'\'';
...
if( 'add' == $act ) {
    ...
    $get_role_list = 'select `id`,`name` from `'.DB_PREFIX.'role` where business_account = \''.$_SESSION['business_account'].'\' order by `id` asc';
...
if( 'delete' == $act ) {
    ...
    $delete_admin = 'delete from `'.DB_PREFIX.'admin` where business_account = \''.$_SESSION['business_account'].'\' and `account` = \''.$account.'\' limit 1';
