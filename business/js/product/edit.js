...
$(function() {
    ...
    function init() {
        ...
        temp += '<input style="cursor: pointer;display: none" class="text-input" type="button" id="add-attr" value="增加属性"/>';
        temp += '<p class="clear" id="show-attr"></p>';
        $('#product_attr').append(temp);

        temp = '<div class="pull-left" style="width: 80%" id="show-attr-0">';
        ...
        for( var i = 0 ; i < data_product_attr[pid].length; i++ ) {
            ...
            temp += '<label class="l-title">'+data_product_attr[pid][i].name+'：</label><em>'+attributes[data_product_attr[pid][i].id]+'</em>';
            switch( data_product_attr[pid][i].type ) {
                case 'text':
                    temp += '<input type="text" class="text-input w50 product-attribute '+required+'" name="attr[0]['+data_product_attr[pid][i].id+']" label="'+data_product_attr[pid][i].name+'" style="border: 1px solid rgb(213, 213, 213); color: rgb(0, 0, 0); display: none;" value="'+attributes[data_product_attr[pid][i].id]+'"/>';
                    break;
                case 'select':
                    temp += '<select name="attr[' + index + '][' + data_product_attr[pid][i].id + ']" class="w100 product-attribute" label="' + data_product_attr[pid][i].name + '" style="display: none" >';
                    var options = data_product_attr[pid][i].options.split(',');
                    for( var j = 0; j < options.length; j++ ) {
                        var selected = '';
                        if( attributes[data_product_attr[pid][i].id] == options[j] ) {
                            selected = 'selected="selected"';
                        }
                        temp += '<option value="' + options[j] + '" '+selected+'>' + options[j] + '</option>';
                    }
                    temp += '</select>';
                    break;
            }
