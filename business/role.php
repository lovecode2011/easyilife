...
$disabled_purview = array('pur_business', 'pur_admin', 'pur_role');
...
//编辑管理员角色
if( 'edit' == $opera ) {
    if( !check_purview('pur_role_edit', $_SESSION['business_purview']) ) {
    ...
    $get_role = 'select * from `'.DB_PREFIX.'role` where business_account = \''.$_SESSION['business_account'].'\' and id = \''.$id.'\' limit 1';
...
//添加管理员角色
if('add' == $opera) {
    if( !check_purview('pur_role_add', $_SESSION['business_purview']) ) {
        show_system_message('权限不足', array());
        exit;
    }
...
if( 'view' == $act ) {
    if( !check_purview('pur_role_view', $_SESSION['business_purview']) ) {
        show_system_message('权限不足', array());
        exit;
    }
...
if( 'add' == $act ) {
    ...
    foreach( $purview as $key => $value ) {
        if( in_array($key, $disabled_purview) ) {
            unset($purview[$key]);
        }
    }
...
if( 'edit' == $act ) {
    ...
    $get_role = 'select `id`,`name`,`purview` from `'.DB_PREFIX.'role` where business_account = \''.$_SESSION['business_account'].'\' and `id`='.$id;
    $role = $db->fetchRow($get_role);
...
if( 'delete' == $act ) {
    if( !check_purview('pur_role_del', $_SESSION['business_purview']) ) {
        show_system_message('权限不足', array());
        exit;
    }
    ...
    $checkAdminRole = 'select `account` from `'.DB_PREFIX.'admin` where business_account = \''.$_SESSION['business_account'].'\' and `role_id`='.$id;
    if( $db->fetchAll($checkAdminRole) ) {
        show_system_message('该角色还有管理员在使用，不能删除', array());
        exit;
    } else {
        $delete_role = 'delete from `'.DB_PREFIX.'role` where business_account = \''.$_SESSION['business_account'].'\' and `id`='.$id.' limit 1';
