...
if($parent_id == 0)
{
    $user = array(
        'nickname' => '圣宝先',
        'headimg' => 'themes/sbx/images/sbx.jpg'
    );
    ...
    $get_product_list = 'select `name`,if(`promote_end`>'.$now.',`promote_price`,`price`) as `price`,`id`,`img` from '.$db->table('product').' where `product_sn` in (\''.$product_sn_str.'\')';
}
...
//获取新添加的产品
$get_product_list = 'select `name`,if(`promote_end`>'.$now.',`promote_price`,`price`) as `price`,`id`,`img` from '.$db->table('product').' where `add_time`>'.(time()-3600*24*7);
$new_product = $db->fetchAll($get_product_list);
assign('new_product_count', count($new_product));

//获取猜你喜欢
$get_fav_products = 'select `name`,if(`promote_end`>'.$now.',`promote_price`,`price`) as `price`,`img`,`id` from '.$db->table('product').' order by `add_time` DESC limit 3';
